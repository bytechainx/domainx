...
use crate::{Decimal, DecimalError, Order, OrderType, TimeInForce, Timestamp};
use thiserror::Error;
...
pub enum ValidationError {
    /// 数量字段为负或关系不成立。
    #[error("数量不变量违反: {0}")]
    Quantity(String, #[source] Option<DecimalError>),
    /// 价格/止损价与订单类型不匹配。
    #[error("价格字段不变量违反: {0}")]
    Price(String),
    /// 时间戳顺序或 GTD 截止不合法。
...
pub fn validate_non_negative_quantities(
    ...
    if quantity < Decimal::ZERO {
        return Err(ValidationError::Quantity(format!("quantity 不能为负: {quantity}"), None));
    }
    if filled_quantity < Decimal::ZERO {
        return Err(ValidationError::Quantity(
            format!("filled_quantity 不能为负: {filled_quantity}"),
            None,
        ));
    }
    if remaining_quantity < Decimal::ZERO {
        return Err(ValidationError::Quantity(
            format!("remaining_quantity 不能为负: {remaining_quantity}"),
            None,
        ));
    }
    Ok(())
...
pub fn validate_quantity_balance(
    ...
    // decimalx 默认关闭 panicking-ops；数量关系用 checked_add。
    let sum = filled_quantity.checked_add(remaining_quantity).map_err(|e| {
...
pub fn validate_order_prices(
    ...
    for (field, value) in [("price", price), ("stop_price", stop_price)] {
        if let Some(value) = value
            && value < Decimal::ZERO
        {
            return Err(ValidationError::Price(format!("{field} 不能为负: {value}")));
        }
    }
...
mod tests {
    use super::*;
    use crate::{InstrumentKey, OrderSide, OrderStatus, TimeInForce};
    ...
    /// DX-VAL-002
    #[test]
    fn dx_val_002_rejects_unbalanced_fill() {
        let err =
    ...
    /// DX-VAL-002
    #[test]
    fn dx_val_002_accepts_balanced() {
    ...
    /// DX-VAL-003
    #[test]
    fn dx_val_003_market_rejects_price() {
    ...
    /// DX-VAL-003
    #[test]
    fn dx_val_003_limit_requires_price() {
        let err =
    ...
    /// DX-VAL-003
    #[test]
    fn dx_val_003_stop_limit_requires_both() {
    ...
    /// DX-VAL-003
    #[test]
    fn dx_val_003_stop_market_rules() {
    ...
    /// DX-VAL-003
    #[test]
    fn dx_val_003_rejects_negative_prices() {
        let err = validate_order_prices(&OrderType::Limit, Some(Decimal::new(-1, 0)), None)
            .expect_err("negative limit price");
        assert!(matches!(err, ValidationError::Price(_)));

        let err = validate_order_prices(&OrderType::StopMarket, None, Some(Decimal::new(-1, 0)))
            .expect_err("negative stop price");
        assert!(matches!(err, ValidationError::Price(_)));
    }

    /// DX-VAL-004
    #[test]
    fn dx_val_004_rejects_created_after_updated() {
    ...
    /// DX-VAL-005
    #[test]
    fn dx_val_005_rejects_gtd_before_created() {
